        ...
        CREATE PROCEDURE [0d2c47cd-6740-4e30-9d88-b815bab60653].[sp_planning_di_row_values_batch_delete] 
        ...
        SET NOCOUNT ON;
          ...
          SET @sql = @sql + '
            DELETE [0d2c47cd-6740-4e30-9d88-b815bab60653].' + QUOTENAME(@DIVTableName) + '
            FROM [0d2c47cd-6740-4e30-9d88-b815bab60653].' + QUOTENAME(@DIVTableName) + '
            INNER JOIN [0d2c47cd-6740-4e30-9d88-b815bab60653].' + QUOTENAME(@TempTableName) + ' AS temp ON temp.rowId = [0d2c47cd-6740-4e30-9d88-b815bab60653].' + QUOTENAME(@DIVTableName) + '.rowId;';
          ...
          SET @sql = @sql + N'
            ...
            -- Insert the incoming values into the temporary table
            INSERT INTO [0d2c47cd-6740-4e30-9d88-b815bab60653].' + QUOTENAME(@DataInputRowTempTableName) + N'
            SELECT value as rowId FROM OPENJSON(@DataInputRowIds);

            UPDATE [0d2c47cd-6740-4e30-9d88-b815bab60653].data_input_row SET status = 500 FROM [0d2c47cd-6740-4e30-9d88-b815bab60653].data_input_row inner join [0d2c47cd-6740-4e30-9d88-b815bab60653].' + @DataInputRowTempTableName + ' as temp on temp.rowId = data_input_row.id;
