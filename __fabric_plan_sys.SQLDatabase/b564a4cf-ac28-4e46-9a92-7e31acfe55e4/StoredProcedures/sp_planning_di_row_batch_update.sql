            ...
            CREATE PROCEDURE [b564a4cf-ac28-4e46-9a92-7e31acfe55e4].[sp_planning_di_row_batch_update] 
            ...
            DECLARE @TempTableName NVARCHAR(128) = '#CR_UPDATE_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
              ...
              -- Insert data into the temporary table using dynamic SQL
                ...
                SET @sql = @sql + 'UPDATE [b564a4cf-ac28-4e46-9a92-7e31acfe55e4].data_input_row 
                SET 
                    ...
                    updatedBy = @updatedBy,
                    updatedAt = @updatedAt
                FROM [b564a4cf-ac28-4e46-9a92-7e31acfe55e4].data_input_row
                INNER JOIN [b564a4cf-ac28-4e46-9a92-7e31acfe55e4].' +  @TempTableName + ' as temp on temp.id = data_input_row.id;';
