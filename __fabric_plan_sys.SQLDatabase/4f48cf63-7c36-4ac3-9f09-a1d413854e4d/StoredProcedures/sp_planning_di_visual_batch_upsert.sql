        ...
        CREATE   PROCEDURE [4f48cf63-7c36-4ac3-9f09-a1d413854e4d].[sp_planning_di_visual_batch_upsert]
        ...
        BEGIN
          ...
          SELECT @UpdateSetClause = STRING_AGG(
            CASE 
              WHEN JSON_VALUE(value, '$.isMetaColumn') = 'true' THEN
                QUOTENAME(JSON_VALUE(value, '$.name')) + ' = [4f48cf63-7c36-4ac3-9f09-a1d413854e4d].[MergeDIMeasureMeta](div.' + QUOTENAME(JSON_VALUE(value, '$.name')) + ', stg.' + QUOTENAME(JSON_VALUE(value, '$.name')) + ', @UpdatedAt, @UpdatedBy, @UpdatedByUPN)'
              ELSE
          ...
          DECLARE @sql NVARCHAR(MAX) = N'
            ...
            MERGE INTO [4f48cf63-7c36-4ac3-9f09-a1d413854e4d].' + QUOTENAME(@DIVTableName) + ' AS div
            ...
            BEGIN
              SELECT div.*
              FROM [4f48cf63-7c36-4ac3-9f09-a1d413854e4d].' + QUOTENAME(@DIVTableName) + ' AS div
