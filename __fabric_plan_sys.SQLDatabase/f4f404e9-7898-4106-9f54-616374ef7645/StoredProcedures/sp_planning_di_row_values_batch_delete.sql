        ...
        CREATE PROCEDURE [f4f404e9-7898-4106-9f54-616374ef7645].[sp_planning_di_row_values_batch_delete] 
        ...
        SET NOCOUNT ON;
          ...
          SET @sql = @sql + '
            DELETE [f4f404e9-7898-4106-9f54-616374ef7645].' + QUOTENAME(@DIVTableName) + '
            FROM [f4f404e9-7898-4106-9f54-616374ef7645].' + QUOTENAME(@DIVTableName) + '
            INNER JOIN [f4f404e9-7898-4106-9f54-616374ef7645].' + QUOTENAME(@TempTableName) + ' AS temp ON temp.rowId = [f4f404e9-7898-4106-9f54-616374ef7645].' + QUOTENAME(@DIVTableName) + '.rowId;';
          ...
          SET @sql = @sql + N'
            ...
            -- Insert the incoming values into the temporary table
            INSERT INTO [f4f404e9-7898-4106-9f54-616374ef7645].' + QUOTENAME(@DataInputRowTempTableName) + N'
            SELECT value as rowId FROM OPENJSON(@DataInputRowIds);

            UPDATE [f4f404e9-7898-4106-9f54-616374ef7645].data_input_row SET status = 500 FROM [f4f404e9-7898-4106-9f54-616374ef7645].data_input_row inner join [f4f404e9-7898-4106-9f54-616374ef7645].' + @DataInputRowTempTableName + ' as temp on temp.rowId = data_input_row.id;
