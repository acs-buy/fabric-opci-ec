                  ...
                  CREATE PROCEDURE [4f48cf63-7c36-4ac3-9f09-a1d413854e4d].[sp_powertable_approval_request_rows_batch_read] 
              ...
              BEGIN
                  ...
                  if (@columns = '')
                  BEGIN
                   SET @sql = @sql + N'SELECT count(*) as count FROM [4f48cf63-7c36-4ac3-9f09-a1d413854e4d].powertable_approval_request_rows' + @innerJoinCondition + ' where powertable_approval_request_rows.approvalRequestId = @approvalRequestId' + @statusCondition + ';';
                  END
                  ELSE
                  BEGIN
                  SET @sql = @sql + N'SELECT '+ @columns + ' FROM [4f48cf63-7c36-4ac3-9f09-a1d413854e4d].powertable_approval_request_rows' + @innerJoinCondition + ' where powertable_approval_request_rows.approvalRequestId = @approvalRequestId' + @statusCondition + ';';
                  END
