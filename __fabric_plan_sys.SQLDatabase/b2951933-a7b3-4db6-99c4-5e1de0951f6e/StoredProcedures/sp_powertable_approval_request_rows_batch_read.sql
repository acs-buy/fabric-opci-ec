                  ...
                  CREATE PROCEDURE [b2951933-a7b3-4db6-99c4-5e1de0951f6e].[sp_powertable_approval_request_rows_batch_read] 
              ...
              BEGIN
                  ...
                  if (@columns = '')
                  BEGIN
                   SET @sql = @sql + N'SELECT count(*) as count FROM [b2951933-a7b3-4db6-99c4-5e1de0951f6e].powertable_approval_request_rows' + @innerJoinCondition + ' where powertable_approval_request_rows.approvalRequestId = @approvalRequestId' + @statusCondition + ';';
                  END
                  ELSE
                  BEGIN
                  SET @sql = @sql + N'SELECT '+ @columns + ' FROM [b2951933-a7b3-4db6-99c4-5e1de0951f6e].powertable_approval_request_rows' + @innerJoinCondition + ' where powertable_approval_request_rows.approvalRequestId = @approvalRequestId' + @statusCondition + ';';
                  END
