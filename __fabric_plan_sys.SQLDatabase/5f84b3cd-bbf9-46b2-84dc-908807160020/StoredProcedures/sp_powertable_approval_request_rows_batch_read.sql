                  ...
                  CREATE PROCEDURE [5f84b3cd-bbf9-46b2-84dc-908807160020].[sp_powertable_approval_request_rows_batch_read] 
              ...
              BEGIN
                  ...
                  if (@columns = '')
                  BEGIN
                   SET @sql = @sql + N'SELECT count(*) as count FROM [5f84b3cd-bbf9-46b2-84dc-908807160020].powertable_approval_request_rows' + @innerJoinCondition + ' where powertable_approval_request_rows.approvalRequestId = @approvalRequestId' + @statusCondition + ';';
                  END
                  ELSE
                  BEGIN
                  SET @sql = @sql + N'SELECT '+ @columns + ' FROM [5f84b3cd-bbf9-46b2-84dc-908807160020].powertable_approval_request_rows' + @innerJoinCondition + ' where powertable_approval_request_rows.approvalRequestId = @approvalRequestId' + @statusCondition + ';';
                  END
