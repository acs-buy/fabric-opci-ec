        ...
        CREATE PROCEDURE [b2951933-a7b3-4db6-99c4-5e1de0951f6e].[sp_planning_di_row_values_batch_delete] 
        ...
        SET NOCOUNT ON;
          ...
          SET @sql = @sql + '
            DELETE [b2951933-a7b3-4db6-99c4-5e1de0951f6e].' + QUOTENAME(@DIVTableName) + '
            FROM [b2951933-a7b3-4db6-99c4-5e1de0951f6e].' + QUOTENAME(@DIVTableName) + '
            INNER JOIN [b2951933-a7b3-4db6-99c4-5e1de0951f6e].' + QUOTENAME(@TempTableName) + ' AS temp ON temp.rowId = [b2951933-a7b3-4db6-99c4-5e1de0951f6e].' + QUOTENAME(@DIVTableName) + '.rowId;';
          ...
          SET @sql = @sql + N'
            ...
            -- Insert the incoming values into the temporary table
            INSERT INTO [b2951933-a7b3-4db6-99c4-5e1de0951f6e].' + QUOTENAME(@DataInputRowTempTableName) + N'
            SELECT value as rowId FROM OPENJSON(@DataInputRowIds);

            UPDATE [b2951933-a7b3-4db6-99c4-5e1de0951f6e].data_input_row SET status = 500 FROM [b2951933-a7b3-4db6-99c4-5e1de0951f6e].data_input_row inner join [b2951933-a7b3-4db6-99c4-5e1de0951f6e].' + @DataInputRowTempTableName + ' as temp on temp.rowId = data_input_row.id;
