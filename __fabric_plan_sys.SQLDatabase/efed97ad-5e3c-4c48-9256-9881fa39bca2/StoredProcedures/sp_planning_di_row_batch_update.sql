            ...
            CREATE PROCEDURE [efed97ad-5e3c-4c48-9256-9881fa39bca2].[sp_planning_di_row_batch_update] 
            ...
            DECLARE @TempTableName NVARCHAR(128) = '#CR_UPDATE_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
              ...
              -- Insert data into the temporary table using dynamic SQL
                ...
                SET @sql = @sql + 'UPDATE [efed97ad-5e3c-4c48-9256-9881fa39bca2].data_input_row 
                SET 
                    ...
                    updatedBy = @updatedBy,
                    updatedAt = @updatedAt
                FROM [efed97ad-5e3c-4c48-9256-9881fa39bca2].data_input_row
                INNER JOIN [efed97ad-5e3c-4c48-9256-9881fa39bca2].' +  @TempTableName + ' as temp on temp.id = data_input_row.id;';
