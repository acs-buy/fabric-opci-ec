        ...
        CREATE PROCEDURE [abf29c97-6605-42db-bf40-988a0419387f].[sp_planning_di_row_values_batch_delete] 
        ...
        SET NOCOUNT ON;
          ...
          SET @sql = @sql + '
            DELETE [abf29c97-6605-42db-bf40-988a0419387f].' + QUOTENAME(@DIVTableName) + '
            FROM [abf29c97-6605-42db-bf40-988a0419387f].' + QUOTENAME(@DIVTableName) + '
            INNER JOIN [abf29c97-6605-42db-bf40-988a0419387f].' + QUOTENAME(@TempTableName) + ' AS temp ON temp.rowId = [abf29c97-6605-42db-bf40-988a0419387f].' + QUOTENAME(@DIVTableName) + '.rowId;';
          ...
          SET @sql = @sql + N'
            ...
            -- Insert the incoming values into the temporary table
            INSERT INTO [abf29c97-6605-42db-bf40-988a0419387f].' + QUOTENAME(@DataInputRowTempTableName) + N'
            SELECT value as rowId FROM OPENJSON(@DataInputRowIds);

            UPDATE [abf29c97-6605-42db-bf40-988a0419387f].data_input_row SET status = 500 FROM [abf29c97-6605-42db-bf40-988a0419387f].data_input_row inner join [abf29c97-6605-42db-bf40-988a0419387f].' + @DataInputRowTempTableName + ' as temp on temp.rowId = data_input_row.id;
