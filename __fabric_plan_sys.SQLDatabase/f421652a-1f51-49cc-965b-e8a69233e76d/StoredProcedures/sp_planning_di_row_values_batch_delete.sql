        ...
        CREATE PROCEDURE [f421652a-1f51-49cc-965b-e8a69233e76d].[sp_planning_di_row_values_batch_delete] 
        ...
        SET NOCOUNT ON;
          ...
          SET @sql = @sql + '
            DELETE [f421652a-1f51-49cc-965b-e8a69233e76d].' + QUOTENAME(@DIVTableName) + '
            FROM [f421652a-1f51-49cc-965b-e8a69233e76d].' + QUOTENAME(@DIVTableName) + '
            INNER JOIN [f421652a-1f51-49cc-965b-e8a69233e76d].' + QUOTENAME(@TempTableName) + ' AS temp ON temp.rowId = [f421652a-1f51-49cc-965b-e8a69233e76d].' + QUOTENAME(@DIVTableName) + '.rowId;';
          ...
          SET @sql = @sql + N'
            ...
            -- Insert the incoming values into the temporary table
            INSERT INTO [f421652a-1f51-49cc-965b-e8a69233e76d].' + QUOTENAME(@DataInputRowTempTableName) + N'
            SELECT value as rowId FROM OPENJSON(@DataInputRowIds);

            UPDATE [f421652a-1f51-49cc-965b-e8a69233e76d].data_input_row SET status = 500 FROM [f421652a-1f51-49cc-965b-e8a69233e76d].data_input_row inner join [f421652a-1f51-49cc-965b-e8a69233e76d].' + @DataInputRowTempTableName + ' as temp on temp.rowId = data_input_row.id;
