CREATE TABLE [dbo].[ref_forme_conclusion] (
    [code]         VARCHAR (20)   NOT NULL,
    [libelle]      NVARCHAR (200) NOT NULL,
    [reference_id] INT            NULL,
    [favorable]    INT            NOT NULL,
    [ordre]        INT            NOT NULL,
    [modifie_par]  NVARCHAR (400) NULL,
    [modifie_le]   DATETIME2 (3)  NULL,
    CONSTRAINT [pk_ref_forme_conclusion] PRIMARY KEY CLUSTERED ([code] ASC),
    CONSTRAINT [ck_forme_favorable] CHECK ([favorable]=(1) OR [favorable]=(0)),
    CONSTRAINT [fk_forme_reference] FOREIGN KEY ([reference_id]) REFERENCES [dbo].[ref_reference] ([id])
);


GO

CREATE   TRIGGER dbo.[tr_ref_forme_conclusion_garde]
ON dbo.[ref_forme_conclusion]
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    IF dbo.fn_peut_ecrire_referentiel('ref_forme_conclusion',
...
CREATE   TRIGGER dbo.[tr_ref_forme_conclusion_horodatage]
ON dbo.[ref_forme_conclusion]
AFTER UPDATE
AS
BEGIN
    ...
    FROM dbo.[ref_forme_conclusion] t
