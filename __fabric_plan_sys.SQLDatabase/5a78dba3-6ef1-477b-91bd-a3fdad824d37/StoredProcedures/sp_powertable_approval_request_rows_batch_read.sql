                  ...
                  CREATE PROCEDURE [5a78dba3-6ef1-477b-91bd-a3fdad824d37].[sp_powertable_approval_request_rows_batch_read] 
              ...
              BEGIN
                  ...
                  if (@columns = '')
                  BEGIN
                   SET @sql = @sql + N'SELECT count(*) as count FROM [5a78dba3-6ef1-477b-91bd-a3fdad824d37].powertable_approval_request_rows' + @innerJoinCondition + ' where powertable_approval_request_rows.approvalRequestId = @approvalRequestId' + @statusCondition + ';';
                  END
                  ELSE
                  BEGIN
                  SET @sql = @sql + N'SELECT '+ @columns + ' FROM [5a78dba3-6ef1-477b-91bd-a3fdad824d37].powertable_approval_request_rows' + @innerJoinCondition + ' where powertable_approval_request_rows.approvalRequestId = @approvalRequestId' + @statusCondition + ';';
                  END
