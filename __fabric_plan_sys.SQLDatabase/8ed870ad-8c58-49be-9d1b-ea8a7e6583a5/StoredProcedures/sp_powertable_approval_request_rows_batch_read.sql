                  ...
                  CREATE PROCEDURE [8ed870ad-8c58-49be-9d1b-ea8a7e6583a5].[sp_powertable_approval_request_rows_batch_read] 
              ...
              BEGIN
                  ...
                  if (@columns = '')
                  BEGIN
                   SET @sql = @sql + N'SELECT count(*) as count FROM [8ed870ad-8c58-49be-9d1b-ea8a7e6583a5].powertable_approval_request_rows' + @innerJoinCondition + ' where powertable_approval_request_rows.approvalRequestId = @approvalRequestId' + @statusCondition + ';';
                  END
                  ELSE
                  BEGIN
                  SET @sql = @sql + N'SELECT '+ @columns + ' FROM [8ed870ad-8c58-49be-9d1b-ea8a7e6583a5].powertable_approval_request_rows' + @innerJoinCondition + ' where powertable_approval_request_rows.approvalRequestId = @approvalRequestId' + @statusCondition + ';';
                  END
