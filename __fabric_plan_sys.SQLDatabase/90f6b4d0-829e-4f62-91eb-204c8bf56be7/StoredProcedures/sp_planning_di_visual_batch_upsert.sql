        ...
        CREATE   PROCEDURE [90f6b4d0-829e-4f62-91eb-204c8bf56be7].[sp_planning_di_visual_batch_upsert]
        ...
        BEGIN
          ...
          SELECT @UpdateSetClause = STRING_AGG(
            CASE 
              WHEN JSON_VALUE(value, '$.isMetaColumn') = 'true' THEN
                QUOTENAME(JSON_VALUE(value, '$.name')) + ' = [90f6b4d0-829e-4f62-91eb-204c8bf56be7].[MergeDIMeasureMeta](div.' + QUOTENAME(JSON_VALUE(value, '$.name')) + ', stg.' + QUOTENAME(JSON_VALUE(value, '$.name')) + ', @UpdatedAt, @UpdatedBy, @UpdatedByUPN)'
              ELSE
          ...
          DECLARE @sql NVARCHAR(MAX) = N'
            ...
            MERGE INTO [90f6b4d0-829e-4f62-91eb-204c8bf56be7].' + QUOTENAME(@DIVTableName) + ' AS div
            ...
            BEGIN
              SELECT div.*
              FROM [90f6b4d0-829e-4f62-91eb-204c8bf56be7].' + QUOTENAME(@DIVTableName) + ' AS div
