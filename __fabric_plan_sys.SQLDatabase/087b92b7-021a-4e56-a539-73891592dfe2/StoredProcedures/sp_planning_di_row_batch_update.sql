            ...
            CREATE PROCEDURE [087b92b7-021a-4e56-a539-73891592dfe2].[sp_planning_di_row_batch_update] 
            ...
            DECLARE @TempTableName NVARCHAR(128) = '#CR_UPDATE_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
              ...
              -- Insert data into the temporary table using dynamic SQL
                ...
                SET @sql = @sql + 'UPDATE [087b92b7-021a-4e56-a539-73891592dfe2].data_input_row 
                SET 
                    ...
                    updatedBy = @updatedBy,
                    updatedAt = @updatedAt
                FROM [087b92b7-021a-4e56-a539-73891592dfe2].data_input_row
                INNER JOIN [087b92b7-021a-4e56-a539-73891592dfe2].' +  @TempTableName + ' as temp on temp.id = data_input_row.id;';
